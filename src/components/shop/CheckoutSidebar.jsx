...
import { ShieldCheck, Tag, Sparkles, CheckCircle2, X, AlertCircle, ChevronRight } from 'lucide-react';
...
const CheckoutSidebar = ({ cart = [], cartTotal, loading = false }) => {
  ...
  return (
    <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800/80 rounded-3xl p-6 shadow-sm space-y-6">
      ...
      {/* Cart items list */}
      <div className="divide-y divide-slate-100 dark:divide-slate-800 max-h-44 overflow-y-auto space-y-2">
        {cart.map((item) => (
      ...
      {/* Offer Code Input Box & Available Coupons List */}
      <div className="space-y-3 pt-3 border-t border-slate-100 dark:border-slate-800">
        <div className="flex items-center justify-between">
          ...
          {activeSchemes.length > 0 && (
            ...
            >
              ...
              View All Coupons ({activeSchemes.length})
        ...
        {appliedScheme ? (
          ...
          <form onSubmit={handleApplyCoupon} className="flex gap-2">
            ...
            <button
              ...
              className="px-3.5 py-2 bg-indigo-600 hover:bg-indigo-700 disabled:opacity-50 text-white font-semibold text-xs rounded-xl transition-all"
            >
              {applyLoading ? '...' : 'Apply'}
            </button>
          </form>
        ...
        {/* Inline List of Available Coupons */}

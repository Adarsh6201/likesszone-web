...
const OrderSearchSidebar = ({ searchId, onSearchIdChange, onSearchSubmit, orders = [], selectedOrderId, onSelectOrder }) => {
  return (
    <div className="lg:col-span-4 space-y-6">
      ...
      <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-3xl p-5 shadow-sm space-y-4">
        ...
        <form onSubmit={onSearchSubmit} className="relative flex items-center">
          <input
            ...
            className="w-full rounded-xl border border-slate-200 bg-slate-50 pl-4 pr-10 py-2.5 text-xs font-semibold outline-none focus:border-indigo-500 focus:bg-white text-slate-900 dark:border-slate-800 dark:bg-slate-800 dark:text-white dark:focus:bg-slate-900"
          />
          <button
            type="submit"
      ...
      <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-3xl p-5 shadow-sm space-y-4">
        <div>
          <h3 className="text-sm font-bold text-slate-900 dark:text-white">Store Orders ({orders.length})</h3>
        ...
        {orders.length === 0 ? (
          ...
          <div className="divide-y divide-slate-100 dark:divide-slate-800/80 max-h-80 overflow-y-auto space-y-1.5 pr-1">
            {orders.map((ord) => {
              const isSelected = selectedOrderId && String(selectedOrderId).toLowerCase() === String(ord.id).toLowerCase();
              return (
                <button
                  key={ord.id}
                  onClick={() => onSelectOrder(ord.id)}
                  className={`w-full text-left rounded-xl p-3 flex justify-between items-center text-xs font-semibold border transition-all cursor-pointer ${
                    isSelected 
                      ? 'border-indigo-500 bg-indigo-50/30 dark:bg-indigo-950/20 text-indigo-700 dark:text-indigo-300' 
                      : 'border-transparent text-slate-700 hover:bg-slate-50 dark:text-slate-400 dark:hover:bg-slate-800'
                  }`}
                >
                  <div>
                  ...
                  <div className="text-right">
                    <p className="font-extrabold text-slate-950 dark:text-white">₹{(ord.total || 0).toFixed(2)}</p>
